$(document).ready(function () {
    ...
    window.loadForm = function (patientId) {
        client.search({
                ...
                bundle.entry.forEach((patient) => {
                    ...
                    if (patient.resource.hasOwnProperty("identifier")) {
                    ...
                    if (patient.resource.hasOwnProperty("address")) {
    ...
    // Perform a search to Allergy list for a specific patient
    window.observation = function (patientId) {
        client.search({
                type: 'Observation',
                query: {
                    patient: patientId
                ...
                const bundle = res.data;
                $("#badgeObservation").html(res.data.total);

                if (res.data.total > 0) {
                    var resourceObservation = JSON.stringify(bundle, undefined, 4);
                    $('#fhirdatasource').append(resourceObservation);

                    bundle.entry.forEach((observation) => {
                            const observationRow = '<tr><td>' + observation.resource.code.coding[0].display + '</td><td>' + observation.resource.category[0].coding[0].display + '</td><td>' + observation.resource.effectiveDateTime + '</td></tr>';
                            $("#observationTable tbody").append(observationRow);
                    });
                }
            })
            .catch((err) => {
